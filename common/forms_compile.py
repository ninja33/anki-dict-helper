...
def parse_edict(path):
    '''
    To generate a word form dictionary for de-inflection
    parse_edict(path)
    path: wordform file path
    
    wordform file format:
    stem : form1, form2, form3, ...
    '''
    indices = {}
    for line in load_definitions(path):
        segments = line.split(':', 1)
        stem = segments[0].strip()
        def_segments = segments[1].split(',')
